...
<div id="peerChat" class="closed">
  <header>
    <span>🤝 Peer Chat (<?php echo e($room); ?>)</span>
...
<script>
  ...
  async function sendMessage() {
    ...
    inEl.value = '';
    try {
      await fetch(sendU, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' }, body: JSON.stringify({ message: msg }) });
    } catch (e) {
  ...
  broomBtn.addEventListener('click', async () => { if (!confirm('Clear all chat messages?')) return; await fetch(clearU, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>' } }); loadMsgs(); });
</script>
<?php /**PATH D:\Laracast\NCK-helper\resources\views/partials/peer-chat.blade.php ENDPATH**/ ?>